...
const TopicList: FC<Props> = ({ activeTopic, setActiveTopic, assistant }) => {
  const { assistants } = useAssistants()
  const { topics, switchAssistant, updateTopic, updateTopics } = useTopics()
  const { t } = useTranslation()
  const { showTopicTime } = useSettings()

  const { assistant: activeAssistant } = useAssistant(activeTopic.assistantId)
  const currentAssistant = assistant || activeAssistant
  ...
  const handleDeleteAfterShown = useCallback(
    async (topic: Topic, e: React.MouseEvent) => {
      ...
      EventEmitter.emit(EVENT_NAMES.DELETE_TOPIC, topic)
      setDeletingTopicId(null)
    },
    [topics, onClearMessages]
  )
  ...
  const onDeleteTopic = useCallback(async (topic: Topic) => {
    EventEmitter.emit(EVENT_NAMES.DELETE_TOPIC, topic)
  }, [])
